...
# proc parseStmt(p: var Parser): ast.Node =
#   case p.current.kind
#   of lex.TokenKeywordInfer:          # inf x = ...
#     advance(p)
#     let name = p.consume(lex.TokenIdentifier)
#     discard p.consume(lex.TokenOperatorAssignment) # =
#     let value = p.parseExpr()
#     ast.Node(kind: ast.nkVarDecl, varName: name, valueExpr: value)
#   else:
#     p.parseExpr()                 # fallback ekspresi biasa

# # proc parseStmts*(p: var Parser): seq[ast.Node] =
# #   result = @[]
# #   while p.current.kind != lex.TokenEndOfFile:
# #     result.add(p.parseStmt())
# #     # optional: consume newline atau semicolon jika ada
# #     if p.current.kind == lex.TokenNewline:
# #       p.advance()

# proc parse*(toks: seq[lex.TokenObject]): seq[ast.Node] =
